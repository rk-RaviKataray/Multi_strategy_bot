...
token_dict = {
    ...
    'BANKNIFTY_SPOT': {"TOKEN": 0, "LP": 0.0, "POS": "", "PNL": 0.0, "LAST_ENTRY": 0, "EMA": 0, "FCH": 0,
                       "NOE": 0, "BROKERAGE": 0},
    'sensex_SPOT': {"TOKEN": 0, "LP": 0.0, "POS": "", "PNL": 0.0, "LAST_ENTRY": 0, "EMA": 0, "FCH": 0,
                      "NOE": 0, "BROKERAGE": 0}}
                      '''
...
token_dict['SENSEX_SPOT'] = {"TOKEN": 0, "LP": 0.0, "POS": "", "PNL": 0.0, "LAST_ENTRY": 0, "EMA": 0, "FCH": 0,
                               "NOE": 0, "BROKERAGE": 0,"QUANTITY":0}
...
delta_dict_current['SENSEX'] =   {0.3:{'CALL':None, 'PUT':None}, 0.2:{'CALL':None, 'PUT':None}, 0.1:{'CALL':None, 'PUT':None}}
...
delta_dict_expected['SENSEX'] =   {0.3:{'CALL':None, 'PUT':None}, 0.2:{'CALL':None, 'PUT':None}, 0.1:{'CALL':None, 'PUT':None}}
...
SENSEX_TOTAL_PNL = 0
SENSEX_TOTAL_BROKERAGE = 0
SENSEX_NET_PNL = 0
SENSEX_TOTAL_ENTRIES = 0
N_time_ = 0
BN_time_ = 0
SX_time_= 0
...
def socket():
    def socket_open():  # Socket open callback function
        ...
        if subscribe_flag:  # This is used to resubscribe the script when reconnect the socket.
            alice.subscribe(get_subscribe_list(banknifty_atm,nifty_atm,sensex_atm,expiry_banknifty,expiry_nifty,expiry_sensex))
    ...
    def feed_data(message):  # Socket feed data will receive in this callback function
        ...
        if feed_message["t"] == "ck":
            ...
            if Feed["tk"] == '1':
                token_dict['sensex_SPOT']["TOKEN"] = Feed['tk']
                token_dict['sensex_SPOT']["LP"] = float(Feed['lp'])
    ...
    subscribe_list = [alice.get_instrument_by_token('INDICES', 26000), alice.get_instrument_by_token('INDICES', 26009),
                      alice.get_instrument_by_token('INDICES',1)]
...
def stuff():
    ...
    SENSEX_TOTAL_PNL_list = []
    SENSEX_TOTAL_BROKERAGE_list = []
    SENSEX_TOTAL_ENTRIES_list = []
    global SX_pnl_list_for_dynamic_graph 
    global SX_time_
    ...
    for x in token_dict.keys():
        ...
        if x[0] == "N":
            ...
            SENSEX_TOTAL_PNL_list.append(token_dict[x]["PNL"])
            SENSEX_TOTAL_BROKERAGE_list.append(token_dict[x]["BROKERAGE"])
            SENSEX_TOTAL_ENTRIES_list.append(token_dict[x]["NOE"])
    ...
    SENSEX_TOTAL_PNL = round(sum(SENSEX_TOTAL_PNL_list), 2)
    SENSEX_NET_PNL = round((sum(SENSEX_TOTAL_PNL_list) - sum(SENSEXTOTAL_BROKERAGE_list)), 2)
    SX_pnl_list_for_dynamic_graph.append(SENSEX_NET_PNL)

    if not os.path.exists("./instrument_data/SENSEX"):
        os.makedirs("./instrument_data/SENSEX")

    
    if datetime.datetime.now(pytz.timezone('Asia/Kolkata')).second == 00 and SX_time_ != int(datetime.datetime.now(pytz.timezone('Asia/Kolkata')).strftime('%s')):
        with open("./instrument_data/SENSEX/candle_data.jsonl", 'a') as SX_candle_data_file:
            SX_time_ = int(datetime.datetime.now(pytz.timezone('Asia/Kolkata')).strftime('%s'))
            SX_open_ = SX_pnl_list_for_dynamic_graph[0]
            SX_high_ = max(SX_pnl_list_for_dynamic_graph)
            SX_low_= min(SX_pnl_list_for_dynamic_graph)
            SX_close_ = int(SENSEX_NET_PNL)
            SX_pnl_list_for_dynamic_graph.clear()
            SX_candle_data_to_append = {"time": int(SX_time_)+19800, "open": SX_open_, "high": SX_high_, "low":SX_low_, "close": SX_close_}
            json.dump(SX_candle_data_to_append, SX_candle_data_file)
            SX_candle_data_file.write('\n')
            SX_candle_data_file.flush()
            os.fsync(SX_candle_data_file)
    ...
    font_kolor_total_sensex = '#009900' if SENSEX_NET_PNL > 0 else '#fa2723'
    ...
    <tr class="bg-light sticky-top top-0">
            ...
            <td>  {SENSEX_TOTAL_PNL}  </td>
            <td>  {SENSEX_TOTAL_BROKERAGE}  </td>
            <td onclick="showPopup('SENSEX')" style="color:{font_color_sx}">  {SENSEX_NET_PNL}  </td>
            <td>  {SENSEX_TOTAL_ENTRIES}  </td>
        ...
        """.format(NIFTY_TOTAL_PNL=NIFTY_TOTAL_PNL,
                   ...
                   SENSEX_TOTAL_PNL=SENSEX_TOTAL_PNL,
                   SENSEX_TOTAL_BROKERAGE=round(sum(SENSEX_TOTAL_BROKERAGE), 2),
                   SENSEX_NET_PNL=SENSEX_NET_PNL,
                   SENSEX_TOTAL_ENTRIES=sum(SENSEX_TOTAL_ENTRIES_list),
                   font_color_nifty = font_kolor_total_nifty,
                   font_color_bn = font_kolor_total_banknifty,
                   font_color_sx = font_kolor_total_sensex,
                   )
    ...
    <table class="table table-striped">
            <thead>
                <tr>
                    ...
                    <td>  <strong > SX_TOTAL_PNL </strong> </td>
                    <td>  <strong> SX_TOTAL_BROKERAGE </strong> </td>
                    <td>  <strong> SX_NET_PNL </strong> </td>
                    <td>  <strong> SX_TOTAL_ENTRIES </strong> </td>
                </tr>
            </thead>
            {}
        </table>
...
def get_subscribe_list(banknifty_atm,nifty_atm,sensex_atm,expiry_banknifty,expiry_nifty,expiry_sensex):
    subscribe_list = [
                      ...
                      # #NIFTY Hedge CE
                      alice.get_instrument_by_token('INDICES', 1),  # NSE,NIFTY FIN SERVICE,26037

                      alice.get_instrument_for_fno(exch='BFO', symbol='SENSEX', expiry_date=str(expiry_sensex),
                                                   is_fut=False, strike=int(sensex_atm) - 800, is_CE=False), #hegde

                      alice.get_instrument_for_fno(exch='BFO', symbol='SENSEX', expiry_date=str(expiry_sensex),
                                                   is_fut=False, strike=int(sensex_atm) - 400, is_CE=False),

                      alice.get_instrument_for_fno(exch='BFO', symbol='SENSEX', expiry_date=str(expiry_sensex),
                         is_fut=False, strike=int(sensex_atm) - 200, is_CE=False), #hedge

                      alice.get_instrument_for_fno(exch='BFO', symbol='SENSEX', expiry_date=str(expiry_sensex),
                                                   is_fut=False, strike=int(sensex_atm), is_CE=False),
                      alice.get_instrument_for_fno(exch='BFO', symbol='SENSEX', expiry_date=str(expiry_sensex),
                                                   is_fut=False, strike=int(sensex_atm), is_CE=True),

                      alice.get_instrument_for_fno(exch='BFO', symbol='SENSEX', expiry_date=str(expiry_sensex),
                                                   is_fut=False, strike=int(sensex_atm) + 200, is_CE=True),  #hedge

                      alice.get_instrument_for_fno(exch='BFO', symbol='SENSEX', expiry_date=str(expiry_sensex),
                                                   is_fut=False, strike=int(sensex_atm) + 400, is_CE=True),
                                        
                      alice.get_instrument_for_fno(exch='BFO', symbol='SENSEX', expiry_date=str(expiry_sensex),
                                                   is_fut=False, strike=int(sensex_atm) + 800, is_CE=True)

                      #alice.get_instrument_for_fno(exch='NFO', symbol='sensex', expiry_date=str(expiry_sensex[0]),
                      #                             is_fut=False, strike=int(sensex_atm) + 100, is_CE=False),
                      #alice.get_instrument_for_fno(exch='NFO', symbol='sensex', expiry_date=str(expiry_sensex[0]),
                      #                             is_fut=False, strike=int(sensex_atm) - 100, is_CE=True),
...
if __name__ == '__main__':
    ...
    expiry_format_sensex = get_expiry_date_trading_symbol(str(expiry_sensex))
    ...
    while True:
        if datetime.datetime.now(pytz.timezone('Asia/Kolkata')).hour >= 00 \
            ...
            global nifty_atm,banknifty_atm,sensex_atm
            # nifty_atm = int(round(float(token_dict['NIFTY_SPOT']["LP"]), -2))
            # banknifty_atm = int(round(float(token_dict['BANKNIFTY_SPOT']["LP"]), -2))
            # sensex_atm = int(round(float(token_dict['sensex_SPOT']["LP"]), -2))

            nifty_atm = 19800
            banknifty_atm = 44900
            sensex_atm = 66500

            print("Nifty atm = {} \nBanknifty atm = {} \nsensex atm = {}".format(nifty_atm, banknifty_atm, sensex_atm))
            break
    ...
    SX_ATM_CE = check_entries(str(get_symbol('SENSEX', sensex_atm, 0, 'C', expiry_format_sensex)),
                              sensex_quantity,is_hedge=False)
    SX_ATM_PE = check_entries(str(get_symbol('SENSEX', sensex_atm, 0, 'P', expiry_format_sensex)),
                              sensex_quantity,is_hedge=False)
    SX_OTM_CE_200 = check_entries(str(get_symbol('SENSEX', sensex_atm, 200, 'C', expiry_format_sensex)),
                                 sensex_quantity,is_hedge=False)
    SX_OTM_PE_200 = check_entries(str(get_symbol('SENSEX', sensex_atm, -200, 'P', expiry_format_sensex)),
                                 sensex_quantity,is_hedge=False)

    SX_OTM_CE_400 = check_entries(str(get_symbol('SENSEX', sensex_atm, 400, 'C', expiry_format_sensex)),
                                 sensex_quantity,is_hedge=False)
    SX_OTM_PE_400 = check_entries(str(get_symbol('SENSEX', sensex_atm, -400, 'P', expiry_format_sensex)),
                                 sensex_quantity,is_hedge=False)
    ...
    SX_HEDGE_CE_100 = check_entries(str(get_symbol('SENSEX', sensex_atm, 100, 'C', expiry_format_sensex)),
                                 sensex_quantity/2,is_hedge=True)
    SX_HEDGE_PE_100 = check_entries(str(get_symbol('SENSEX', sensex_atm, -100, 'P', expiry_format_sensex)),
                                 sensex_quantity/2,is_hedge=True)
    SX_HEDGE_CE_300 = check_entries(str(get_symbol('SENSEX', sensex_atm, 300, 'C', expiry_format_sensex)),
                                 sensex_quantity/2,is_hedge=True)
    SX_HEDGE_PE_300 = check_entries(str(get_symbol('SENSEX', sensex_atm, -300, 'P', expiry_format_sensex)),
                                 sensex_quantity/2,is_hedge=True)

    SX_HEDGE_CE = check_entries(str(get_symbol('SENSEX', sensex_atm, 800, 'C', expiry_format_sensex)),
                                 sensex_quantity/2,is_hedge=True)
    SX_HEDGE_PE = check_entries(str(get_symbol('SENSEX', sensex_atm, -800, 'P', expiry_format_sensex)),
                                 sensex_quantity/2,is_hedge=True)
    ...
    SX_call_list = [SX_ATM_CE, SX_OTM_CE_200,SX_OTM_CE_400
                    # , FN_ITM_CE_50
                    ]
    SX_put_list = [SX_ATM_PE,SX_OTM_PE_200,SX_OTM_PE_400]
                 
                 

    SX_call_list_HEDGE = [SX_HEDGE_CE, SX_HEDGE_CE_100,SX_HEDGE_CE_300,
    SX_HEDGE_PE, SX_HEDGE_PE_100,SX_HEDGE_PE_300 ]
    ...
    SX_CALL_PROCESS = processing_multi(SX_call_list)
    SX_PUT_PROCESS = processing_multi(SX_put_list)

    SX_CALL_HEDGE_PROCESS = processing_multi(SX_call_list_HEDGE)
    #SX_PUT_HEDGE_PROCESS = processing_multi(FN_put_list_HEDGE)
    ...
    SX_CALL_PROCESS.start()
    SX_PUT_PROCESS.start()

    SX_CALL_HEDGE_PROCESS.start()
