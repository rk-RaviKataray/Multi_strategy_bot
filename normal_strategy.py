...
class normal_strategy(threading.Thread):
    ...
    def run(self):
        ...
        token_dict[self.symbol][strategy_name.NORMAL.value]['PNL'] = 0.0
        token_dict[self.symbol][strategy_name.NORMAL.value]['LAST_ENTRY'] = 0.0
        token_dict[self.symbol][strategy_name.NORMAL.value]['NOE'] = 0
        token_dict[self.symbol][strategy_name.NORMAL.value]['BROKERAGE'] = 0.0
        token_dict[self.symbol][strategy_name.NORMAL.value]['POS'] = ""
        token_dict[self.symbol][strategy_name.DATA.value]["FCH"] = 0.0
        ...
        while True:
            ...
            while True:
                token_dict[self.symbol][strategy_name.NORMAL.value]['PNL'] += 1
                token_dict[self.symbol][strategy_name.NORMAL.value]['LAST_ENTRY'] += 0.0
                token_dict[self.symbol][strategy_name.NORMAL.value]['NOE'] += 0
                token_dict[self.symbol][strategy_name.NORMAL.value]['BROKERAGE'] +=0.0
                #print(token_dict)
            ...
            while start_time < \
                ...
                token_dict[self.symbol][strategy_name.DATA.value]["EMA"] = token_dict[self.symbol][strategy_name.DATA.value]["EMA"]

                token_dict[self.symbol][strategy_name.NORMAL.value]["BROKERAGE"] = self.short_brokerage + self.long_brokerage

                if self.lng == True:
                    token_dict[self.symbol][strategy_name.NORMAL.value][
                    ...
                    token_dict[self.symbol][strategy_name.NORMAL.value][
                        'PNL'] = self.long_pnl_booked + self.short_pnl_booked + (
                            (token_dict[self.symbol][strategy_name.NORMAL.value]['LAST_ENTRY'] - float(
    ...
    def go_long(self, pivot, reason):
        ...
        if (float(token_dict[self.symbol][strategy_name.DATA.value]["LP"]) > pivot) and self.lng == False:
            ...
            if (float(token_dict[self.symbol][strategy_name.DATA.value]["LP"]) > pivot) and self.lng == False:
                ...
                token_dict[self.symbol][strategy_name.NORMAL.value]["NOE"] = \
                token_dict[self.symbol][strategy_name.NORMAL.value]["NOE"] + 1
                self.lng = True
                token_dict[self.symbol][strategy_name.NORMAL.value]["POS"] = "LONG"
                # self.sht = False
                self.lng_counter = 0
                print('{} went long at price-{}, time-{}:{}:{}'.format(self.symbol,
                                                                       ...
                                                                       datetime.datetime.now(
                                                                           pytz.timezone(
                                                                               'Asia/Kolkata')).second))
                token_dict[self.symbol][strategy_name.NORMAL.value]['LAST_ENTRY'] = self.price
    ...
    def go_short(self, pivot, reason):
        ...
        if self.first_trade:
            ...
            self.sht = True
            token_dict[self.symbol][strategy_name.NORMAL.value]["POS"] = "SHORT"
            token_dict[self.symbol][strategy_name.NORMAL.value]["NOE"] = token_dict[self.symbol][strategy_name.NORMAL.value]["NOE"] + 1
            token_dict[self.symbol][strategy_name.NORMAL.value]["LAST_ENTRY"] = self.price
            ...
            if (token_dict[self.symbol][strategy_name.DATA.value]["LP"] < pivot):
                ...
                print('{} went short at price-{}, time-{}:{}:{}'.format(self.symbol,
                                                                        ...
                                                                        datetime.datetime.now(
                                                                            pytz.timezone(
                                                                                'Asia/Kolkata')).second))
                token_dict[self.symbol][strategy_name.NORMAL.value]['LAST_ENTRY'] = float(token_dict[self.symbol][strategy_name.DATA.value]["LP"])
    ...
    def hedge(self):
        ...
        token_dict[self.symbol][strategy_name.NORMAL.value]['LAST_ENTRY'] = self.price
        token_dict[self.symbol][strategy_name.NORMAL.value]["LONG"] = True
        token_dict[self.symbol][strategy_name.NORMAL.value]["NOE"] = \
        token_dict[self.symbol][strategy_name.NORMAL.value]["NOE"] + 1
        self.lng = True
        token_dict[self.symbol][strategy_name.NORMAL.value]['POS'] = 'LONG'
        ...
        while start_time <= time_now <= end_time:
            ...
            if self.lng == True:
                token_dict[self.symbol][strategy_name.DATA.value]['PNL'] = (
                        (token_dict[self.symbol][strategy_name.DATA.value]['LP'] -
                         token_dict[self.symbol][strategy_name.NORMAL.value]['LAST_ENTRY']) * self.quantity)
